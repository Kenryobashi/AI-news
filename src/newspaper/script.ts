...
export async function generateScript(
  ...
  const sportsNews = [
    ...sports.worldcup.map((s) => `⚽ワールドカップ: ${s.title} ${s.description}`),
  ...
  const prompt = `あなたは朝の通勤ラジオのパーソナリティです。
聴いている人は鉄鋼会社の営業マンで、毎朝30分の通勤中に車で聴いています。

以下のデータをもとに、まるで親しい先輩が話しかけるような自然な標準語の口語で原稿を書いてください。
方言（九州弁・博多弁など）は絶対に使わないこと。
...
7. スポーツコーナー（2〜3分）：
   - ワールドカップ情報がある場合は**最優先で先に読む**。日本代表の試合結果・得点者・次戦の相手を盛り込む
   - 「昨日の試合、見ましたか？」のような視聴者に語りかける口調で
   - 続いて横浜DeNA・ギラヴァンツ・ライジングゼファーの結果を気軽に紹介
   - 情報がない場合は「昨日は試合がなかったですね、次の試合が楽しみです」程度でOK
8. 締め（30秒）：短く元気よく。今日も頑張れる一言で終わる

【固有名詞の読み方】
- 日本製鉄 → 「にっぽんせいてつ」（「にほんせいてつ」ではない）
- JFE → 「ジェイエフイー」
- 神戸製鋼 → 「こうべせいこう」
